...
user_name="Alice"
topic="Dog"
prompt=f"你好{user_name}, 我想问你关于{topic}的问题。"
```
**问题**
 - 难以维护和修改
 - 容易出现格式错误
...
template=PromptTemplate.from_template(
    "你好{user_name}, 我想问你关于{topic}的问题。"
)
prompt=template.format(username="Alice", topic="Dog")
```
**优势:**
- **可复用** - 一个模板，多次使用
- **可维护** - 模板和数据分离
...
template=PromptTemplate(
    input_variables=["变量名1", "变量名2"],
    template="你的模板文本，{变量名1} {变量名2}"
)
```
#### 创建模板的三种方法
**方法1:from_template(最简单，推荐)**
...
template=PromptTemplate(
    ...
    template="为{product}写一句广告词，重点突出{feature}。"
     )
```
**方法 3：部分变量预填充**
