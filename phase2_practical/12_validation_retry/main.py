...
def example_1_with_retry():
    ...
    """
    print("\n" + "=" * 70)
    print("示例 1：使用 with_retry() - 自动重试机制")
    print("=" * 70)

    # 创建带重试的LLM
    llm_with_retry=model.with_retry(
        retry_if_exception_type=(ConnectionError,TimeoutError), # 重试的异常类型
        wait_exponential_jitter=True, # 是否使用指数退避+随机抖动
        stop_after_attempt=3, # 最大尝试次数3次
    )
    print("\n配置:")
...
def example_2_with_fallbacks():
    ...
    """
    print("\n" + "=" * 70)
    print("示例 2：使用 with_fallbacks() - 降级/备用方案")
    print("=" * 70)

    # 创建主模型和备用模型
    model_primary=model
    # 创建备用模型
    model_fallback=init_chat_model("deepseek:deepseek-chat",api_key=KEY)

    llm_with_fallbacks =model_primary.with_fallbacks([model_fallback])
    print("\n配置:")
    print("  - 主模型: deepseek:deepseek-chat")
    print("  - 备用模型: deepseek:deepseek-chat")
    print("\n调用 LLM (如果失败会自动切换到备用模型)...")
    try:
...
def example_3_pydantic_validation():
    ...
    print("\n测试1 :有效数据")
    try:
        user=User(name="张三",age=18,email="123@123.com")
    ...
    print("\n测试2 :年龄超过限制")
    try:
        user=User(name="张三",age=200,email="123@123.com")
        print(f"验证通过:{user.name},{user.age},{user.email}")
    except ValidationError as e:
        print(f"验证失败: {e}")
    ...
    print("\n测试3 :邮箱格式错误")
    try:
        user=User(name="张三",age=18,email="123")
        print(f"验证通过:{user.name},{user.age},{user.email}")
    except ValidationError as e:
        print(f"验证失败: {e}")
...
class Product(BaseModel):
    ...
    price:float =Field(description="产品价格(数字类型)",gt=0)
    stock:int =Field(description="库存数量(数字类型)",gt=0)

    @field_validator('name')
    @classmethod
    def validate_name(cls,v):
        if v.lower() =="unknown":
            raise ValueError("产品名称不能为Unknown")
        return v
...
def example_4_llm_validation_retry():
    ...
    如果LLM输出不符合验证规则，重新提示并重试
    """
    print("\n" + "=" * 70)
    print("示例 4：LLM 输出验证 + 重试")
    print("=" * 70)
    ...
    text="iphone 15 售价5999元 ，库存50件"

    print(f"\n提取文本:{text}")
    print(f"验证规则:price>0,stock>0,name不能是'unknown'")
    ...
    for attempt in range(1,max_retries+1):
        ...
        try:
            ...
            重要：price必须是数字类型(不是字符串)，stock必须是整数类型
            文本:{text}"""
            ...
            print(f"产品价格:{result.price}")
            print(f"库存数量:{result.stock}")
            break
        except ValidationError as e:
...
def validate_article(article:Article)->bool:
    """
    自定义验证逻辑
    检查 word_count 是否与content 实际字数接近
    """
...
class ExtractedData(BaseModel):
    ...
    value:str=Field(description="数值(数字类型，必须>0)",gt=0)
